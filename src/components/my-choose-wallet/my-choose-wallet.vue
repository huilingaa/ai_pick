	...
	<view>
		...
		<my-drop-down-box title="Digital wallets" ref="cmpwalletpopup" leftIcon="close">
			<view class="walletBox">
				<view>
					<view class="walletBox_item" v-for="(item,index) in walletList" :key="index"
						@click="handleSwitchWallet(item.id)" :class="{active: item.id === currentWallet.id}">
						<view class="walletBox_item-info">
							<view class="walletBox_item-label">
								<view>{{item.name}}</view>
								<view class="walletBox_item-tag" v-if="item.id === currentWallet.id">
									{{$t('currentWallet'||'当前节点')}}
								</view>
								<view class="walletBox_item-type" v-if="!item.inviteCode">{{$t('notActive'||'未激活')}}
								</view>
							</view>
